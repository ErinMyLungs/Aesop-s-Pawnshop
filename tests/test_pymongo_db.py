...
class TestDBInit:
    ...
    def test_init_db_nondefault_collection(self):
        ...
        expected_index = {
            "_id_": {"v": 2, "key": [("_id", 1)], "ns": "db.foobar"},
            "post_id_text": {
                "v": 2,
                "unique": True,
                "key": [("post_id", "text")],
    ...
    def test_init_db_non_default_index(self):
        ...
        index_name = "fizzbuzz"
        expected_index = {
            "_id_": {"v": 2, "key": [("_id", 1)], "ns": "db.fizzbuzz"},
            "fizzbuzz_text": {
                "v": 2,
                "unique": True,
                "key": [("fizzbuzz", "text")],
...
class TestInsertClass:
    ...
    def test_insert_empty_submission(self):
        assert (
            pymongo_db.insert_reddit_submission_dict(self.submission, self.collection)
            == "Error, submission object is None"
        )
    ...
    def test_insert_example_submission(self):
        ...
        assert (
            self.collection.find_one({"post_id": "d2awru"})["post_id"]
            == submission["post_id"]
        )
