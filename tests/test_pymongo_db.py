...
def test_insert_empty_submission():
    db = mongomock.MongoClient().db
    coll = db['reddit']
    submission = None

    assert pymongo_db.insert_reddit_submission_dict(submission, coll) == "Error, submission object is None"

@mongomock.patch(on_new='create')

def test_insert_example_submission():
    db = mongomock.MongoClient().db
    coll = db['reddit']
    ...
    pymongo_db.insert_reddit_submission_dict(submission, coll)
    assert coll.find_one({'post_id':'d2awru'})['post_id'] == submission['post_id']